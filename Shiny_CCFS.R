...
bird.weather<-read.csv("data/bird.socioenviro.csv", stringsAsFactors = F) %>% filter(param.descr %in% c("Precipitation (mm day-1)", "Temperature at 2 Meters (C)", "Wind Speed at 2 Meters (m/s)", "Population (persons)", "Per capita personal income"))
##format date
